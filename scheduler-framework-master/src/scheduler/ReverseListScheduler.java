...
public class ReverseListScheduler extends Scheduler {
	 ...
	 */
	public ReverseListScheduler(RC rc) {
...
/**
 * TODO modify 
 * -> candidates are nodes with no unhandled SUCCESSORS
 * -> time step to negative values, interval should work properly 
 * -> after all nodes scheduled, shift the intervals to start at t=0
 * 
 */
						...
						if(n.getRT().equals(k) && n.bottom()) {
							candidates.put(n, new Interval(t-n.getDelay()+1,t)); // put to candidates as it would ENDS now 
...
/**
 *  TODO 
 *  determine schedule length and shift intervals
 *  
 */
		int length = schedule.length() - 1;
